...
CClientUserItem::CClientUserItem()
{
	...
	ZeroMemory(&m_UserTaskList,sizeof(m_UserTaskList));
...
tagUserTaskInfo* CClientUserItem::GetUserTaskInfo(enTaskKind enTK)
{
	tagUserTaskList* pTaskList = GetUserTaskList();
	//遍历任务，后面改为map方式
	switch ( enTK )
	{
	case enTaskKind_WinTask:
		{
			for ( BYTE i=0; i<pTaskList->wCount;++i )
			{
				if ( !_tcscmp(pTaskList->TaskInfo[i].szTaskKind,L"WinTask") )
					return &pTaskList->TaskInfo[i];
			}
		}
		break;
	case  enTaskKind_CardTask:
		{
			for ( BYTE i=0; i<pTaskList->wCount;++i )
			{
				if ( !_tcscmp(pTaskList->TaskInfo[i].szTaskKind,L"CardTask") )
					return &pTaskList->TaskInfo[i];
			}
		}
		break;
...
bool CPlazaUserManager::UpdateUserTaskList(IClientUserItem * pIClientUserItem, tagUserTaskList * pUserTaskList)
{
	tagUserTaskList* pUserTask = pIClientUserItem->GetUserTaskList();

	pUserTask->wCount = pUserTaskList->wCount;
	WORD wSize = pUserTaskList->wCount * sizeof(pUserTaskList->TaskInfo[0]);
	CopyMemory(pUserTask->TaskInfo,pUserTaskList->TaskInfo,wSize);

	ASSERT(m_pIUserManagerSink!=NULL);
	if (m_pIUserManagerSink!=NULL) m_pIUserManagerSink->OnUserTaskUpdate(pIClientUserItem);
...
//
bool CGameUserManager::UpdateUserTaskList(IClientUserItem * pIClientUserItem, tagUserTaskList * pUserTaskList)
{
	tagUserTaskList* pUserTask = pIClientUserItem->GetUserTaskList();
