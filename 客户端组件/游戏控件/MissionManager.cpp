...
bool CMissionSocket::SwitchServerItem()
{
	...
	if (m_bTryDefault==false)
	{
		...
		szFileName.Format(TEXT("%s/Platform.ini"),szPath);

		TCHAR szServerAddr[16] = {0};
		GetPrivateProfileString(TEXT("LogonServer"),TEXT("ServerAddr"),TEXT(""),szServerAddr,CountArray(szServerAddr),szFileName);

		lstrcpyn(m_szCurrentServer,szServerAddr,CountArray(m_szCurrentServer));
