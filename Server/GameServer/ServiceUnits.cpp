...
CServiceUnits::CServiceUnits()
{
	...
	//设置对象
	ASSERT_ERR(g_pServiceUnits==NULL);
...
{
	//效验状态
	ASSERT_ERR(m_ServiceStatus==ServiceStatus_Stop);
...
bool CServiceUnits::CreateServiceDLL()
{
	...
	{
		TRACE_LOG(m_TimerEngine.GetErrorDescribe(),TraceLevel_Exception);
		return false;
	}
	...
	{
		TRACE_LOG(m_AttemperEngine.GetErrorDescribe(),TraceLevel_Exception);
		return false;
	}
	...
	{
		TRACE_LOG(m_TCPSocketService.GetErrorDescribe(),TraceLevel_Exception);
		return false;
	}
	...
	{
		TRACE_LOG(m_TCPNetworkEngine.GetErrorDescribe(),TraceLevel_Exception);
		return false;
	}
	...
	{
		TRACE_LOG(m_KernelDataBaseEngine.GetErrorDescribe(),TraceLevel_Exception);
		return false;
	}
	...
	{
		TRACE_LOG(m_RecordDataBaseEngine.GetErrorDescribe(),TraceLevel_Exception);
		return false;
	}
	...
	{
		TRACE_LOG(m_GameServiceManager.GetErrorDescribe(),TraceLevel_Exception);
		return false;
	}
	...
	if ((m_GameServiceOption.wServerType&GAME_GENRE_MATCH)!=0)
	{
		...
		{
			TRACE_LOG(m_GameMatchServiceManager.GetErrorDescribe(),TraceLevel_Exception);
			return false;
		}
	}
...
bool CServiceUnits::RectifyServiceParameter()
{
	...
	{
		TRACE_LOG(TEXT("游戏模块的绑定 DLL 名字与配置模块 DLL 名字不一致"),TraceLevel_Exception);
		return false;
	}

	//调整参数
	if (m_GameServiceOption.dwRoomID==0 && m_GameServiceManager->RectifyParameter(m_GameServiceOption)==false)
	{
		TRACE_LOG(TEXT("游戏模块调整配置参数失败"),TraceLevel_Exception);
		return false;
	}
	...
	{
		TRACE_LOG(TEXT("游戏模块不支持配置中指定的房间模式类型"),TraceLevel_Exception);
		return false;
	}
	...
	//游戏记录
	if (m_GameServiceOption.wServerType&(GAME_GENRE_GOLD|GAME_GENRE_MATCH|GAME_GENRE_SCORE))
	{
	...
	//最小积分
	if (m_GameServiceOption.wServerType&(GAME_GENRE_GOLD|GAME_GENRE_SCORE))
	{
...
bool CServiceUnits::SetServiceStatus(enServiceStatus ServiceStatus)
{
	...
	if (m_ServiceStatus!=ServiceStatus)
	{
		...
		if ((m_ServiceStatus!=ServiceStatus_Service)&&(ServiceStatus==ServiceStatus_Stop))
		{
			...
			TRACE_LOG(pszString,TraceLevel_Exception);
		}
		...
		//状态通知
		ASSERT_ERR(m_pIServiceUnitsSink!=NULL);
...
{
	//状态效验
	ASSERT_ERR(m_AttemperEngine.GetInterface()!=NULL);
...
bool CServiceUnits::LoadDataBaseParameter(LPCTSTR pszDataBaseAddr, LPCTSTR pszDataBaseName, tagDataBaseParameter & DataBaseParameter)
{
	...
	try
	{
		...
		if (PlatformDBAide.ExecuteProcess(TEXT("GSP_GS_LoadDataBaseInfo"),true)!=DB_SUCCESS)
		{
			...
			//提示消息
			TRACE_LOG(szErrorDescribe,TraceLevel_Exception);
...
LRESULT CServiceUnits::OnUIControlRequest(WPARAM wParam, LPARAM lParam)
{
	...
	case UI_CORRESPOND_RESULT:		//协调成功
		{
			//效验消息
			ASSERT_ERR(DataHead.wDataSize==sizeof(CP_ControlResult));
	...
	case UI_SERVICE_CONFIG_RESULT:	//配置结果
		{
			...
			if ((m_ServiceStatus!=ServiceStatus_Service)&&(pControlResult->cbSuccess==ER_SUCCESS))
			{
				...
				//I_TRACE(TEXT("底注[%I64d],准入[%I64d]"),m_GameServiceOption.lCellScore,m_GameServiceOption.lMinEnterScore);
			}
