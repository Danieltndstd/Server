...
VOID CDlgInsureSave::OnBnClickedTakeScore()
{
	...
	if (lTakeScore<=0L)
	{
		...
		Information.ShowMessageBox(TEXT("取出的元宝数量不能为空，请重新输入元宝数量！"),MB_ICONERROR,30);
	...
	if (lTakeScore>m_pUserInsureInfo->lUserInsure)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("您保险柜元宝的数目余额不足，请重新输入元宝数量！"),MB_ICONERROR,30);
...
VOID CDlgInsureSave::OnBnClickedSaveScore()
{
	...
	if (lSaveScore<=0L)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("存入的元宝数量不能为空，请重新输入元宝数量！"),MB_ICONERROR,30);
	...
	if (lSaveScore>m_pUserInsureInfo->lUserScore)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("您所携带元宝的数目余额不足，请重新输入元宝数量！"),MB_ICONERROR,30);
...
VOID CDlgInsureSave::OnBnClickedForgetInsure()
{
	...
	_sntprintf(szLogonLink,CountArray(szLogonLink),TEXT("%s/LogonLink4.htm"),szPlatformLink);
...
VOID CDlgInsureTransfer::OnBnClickedTransferScore()
{
	...
	if (m_pUserInsureInfo->lUserInsure < m_pUserInsureInfo->lTransferPrerequisite)
	{
		...
		_sntprintf(szTipInfo,CountArray(szTipInfo),TEXT("您保险柜元宝的数目余额不足最低转账条件 %I64d，无法进行转账操作！"),m_pUserInsureInfo->lTransferPrerequisite);
		CInformation Information(this);
	...
	if (lTransferScore<=0L)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("赠送的元宝数量不能为空，请重新输入元宝数量！"),MB_ICONERROR,30);
	...
	if (lTransferScore<m_pUserInsureInfo->lTransferPrerequisite)
	{
		//提示消息
		TCHAR szTipInfo[128]={0};
		_sntprintf(szTipInfo,CountArray(szTipInfo),TEXT("从保险柜转账的元宝数目不能少于 %I64d，无法进行转账操作！"),m_pUserInsureInfo->lTransferPrerequisite);
		CInformation Information(this);
		Information.ShowMessageBox(szTipInfo,MB_ICONERROR,30);
...
BOOL CDlgInsureMain::OnInitDialog()
{
	...
	m_TabControl.InsertItem(INSURE_TRANSFER,TEXT(" 赠送元宝 "));
...
// 	pDC->FillSolidRect(nLBorder,rcTabControl.bottom-1L,nWidth-nLBorder-nRBorder,1,RGB(145,168,183));
	...
	if (m_bInitInfo==true)
	{
		...
		//绘画提示
		LPCTSTR pszScoreString=TEXT("元  宝：");
		...
		//构造提示
		TCHAR szString[128]=TEXT("");
		_sntprintf(szString,CountArray(szString),TEXT("温馨提示：存入元宝免手续费，取出将扣除%ld‰,赠送将扣除%ld‰的手续费"),m_UserInsureInfo.wRevenueTake,m_UserInsureInfo.wRevenueTransfer);
...
bool CDlgInsurePlaza::OnEventMissionLink(INT nErrorCode)
{
	...
	if (nErrorCode!=0)
	{
		...
		if (m_bSaveMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("连接服务器超时，元宝存入失败，请稍后再次重试！"),MB_OK|MB_ICONSTOP,30);
		}
		...
		if (m_bTakeMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("连接服务器超时，元宝取出失败，请稍后再次重试！"),MB_OK|MB_ICONSTOP,30);
		}
		...
		if (m_bTransferMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("连接服务器超时，元宝赠送失败，请稍后再次重试！"),MB_OK|MB_ICONSTOP,30);
		}
	...
	if (m_bQueryMission==true)
	{
		//设置变量
		CGlobalUserInfo * pGlobalUserInfo=CGlobalUserInfo::GetInstance();
		auto dwUserID = pGlobalUserInfo->GetGlobalUserData()->dwUserID;

		//发送数据
		m_MissionManager.SendData(MDM_GP_USER_SERVICE, SUB_GP_QUERY_INSURE_INFO, &dwUserID, sizeof(dwUserID));
	}
...
bool CDlgInsurePlaza::OnEventMissionShut(BYTE cbShutReason)
{
	...
	if (cbShutReason!=SHUT_REASON_NORMAL)
	{
		...
		if (m_bSaveMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("与服务器的连接异常断开了，元宝存入失败！"),MB_ICONERROR,30);
		}
		...
		if (m_bTakeMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("与服务器的连接异常断开了，元宝取出失败！"),MB_ICONERROR,30);
		}
		...
		case SUB_GP_QUERY_USER_INFO_RESULT:	//用户信息
			{
				...
				_sntprintf(szMessage, CountArray(szMessage), TEXT("您确定要给[%s], ID:%d 赠送%I64d 元宝吗?"), pTransferUserInfo->szNickName, pTransferUserInfo->dwTargetGameID, m_lScore);
