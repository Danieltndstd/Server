...
bool CGameFrameService::OnEventSocket(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	if (wMainCmdID==MDM_GF_GAME)
	{
		...
		switch(wSubCmdID)
		{
		case IPC_SUB_GF_MATCH_READY:
			OutputDebugString(TEXT("比赛开始"));
			CMatchFrameControl::GetInstance()->SetMatchOverStatus(FALSE);
			break;
		case IPC_SUB_GF_MATCH_OUT:
			OutputDebugString(TEXT("玩家淘汰"));
			CMatchFrameControl::GetInstance()->SetMatchOverStatus(TRUE);
			break;
		case IPC_SUB_GF_MATCH_SIGNUP:
			OutputDebugString(TEXT("玩家报名"));
			break;
		case IPC_SUB_GF_MATCH_NULL:
			OutputDebugString(TEXT("玩家退赛"));
			break;
		default:
			break;
		}
	...
	if(wMainCmdID==MDM_GF_FRAME)
	{
		...
		case SUB_GR_MATCH_INFO:				//比赛信息
			{
				//参数效验
				ASSERT(wDataSize==sizeof(CMD_GR_MatchInfoNew));
				if(wDataSize!=sizeof(CMD_GR_MatchInfoNew)) return false;
				
				//变量定义
				CMD_GR_MatchInfoNew *pMatchInfo=(CMD_GR_MatchInfoNew*)pData;

				tagMatchViewInfo MatchViewInfo;
				ZeroMemory(&MatchViewInfo,sizeof(tagMatchViewInfo));
				CopyMemory(&MatchViewInfo,pMatchInfo,sizeof(CMD_GR_MatchInfoNew));

				m_pGameFrameView->SetMatchInfo(NULL);

				m_pGameFrameEngine->OnEventGameMessage(IPC_SUB_GF_MATCH_INFO,&MatchViewInfo,sizeof(tagMatchViewInfo));

				if ( m_pIMatchFrameControl ) m_pIMatchFrameControl->SetMatchViewInfo(&MatchViewInfo);
...
//用户任务
VOID CGameFrameService::OnUserTaskUpdate(IClientUserItem * pIClientUserItem, bool bLookonUser)
{
	m_pGameFrameEngine->OnEventUserScore(pIClientUserItem,bLookonUser);
	return ;
}
...
bool CGameFrameService::CreateGameFrameService()
{
	...
	if ( pServerAttribute->wServerType&GAME_GENRE_MATCH )
		m_pIMatchFrameControl = (IMatchFrameControl*)pGlobalUnits->QueryGlobalModule(MODULE_MATCH_FRAME_CONTROL,IID_IMatchFrameControl,VER_IMatchFrameControl);
	else
