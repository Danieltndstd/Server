...
protected:
	...
	tagUserCardBox					m_UserCardBox;						//对局宝箱
...
protected:
	...
	SCORE							m_lRestrictScore;					//限制积分(每局封顶)
	SCORE	 						m_lFrozenedScore;					//冻结积分(服务费)
	SCORE							m_lFillUserScore;					// 补充用户分数
	BYTE							m_IsAutoAddScore;					// 是否自动补分
	//登录信息
protected:
...
protected:
	BYTE							m_cbBloodRoom;						//血量房标识
	BYTE							m_cbRoomType;						//房间级别
	WORD							m_wCashInRatio;						//血量与金币兑换比率
	BYTE							m_cbAutoAddScore;					//自动加分
	BYTE							m_cbIsAutoVariation;				//补分变更标识
...
protected:
	DWORD							m_dwDrawStartTime;					//游戏开始时间 已作废
	DWORD							m_dwSitDownTime;					//坐下时间
protected:
	BYTE							m_cbBlackUser;						//黑户
	DWORD							m_dwHeartTime;						//手机心跳时间
	BYTE							m_cbEnterTable;						//进桌完成标志
...
public:
	...
	//对局宝箱
	virtual tagUserCardBox* GetUserCardBox() { return &m_UserCardBox; }
...
public:
	...
	// 补充分数
	virtual SCORE GetFillUserScore() { return m_lFillUserScore; }
...
public:
	//设置状态
	virtual bool SetUserStatus(BYTE cbUserStatus, WORD wTableID, WORD wChairID, BYTE cbNotifyClient = TRUE);
	//写入积分
	...
	//写入奖牌
	virtual bool WriteUserMedal(LONG lUserMedal,BYTE cbScoreType);
	//获取奖牌
	virtual LONG GetUserMedal(){return m_UserInfo.dwUserMedal;};
...
public:
	...
	// 补分
	virtual bool FillUserScore(SCORE lScore, BYTE IsAutoAddScore);

	//冻结血量
	virtual bool FrozenedUserBlood(SCORE lScore){return true;}
	//解冻血量
	virtual bool UnFrozenedUserBlood(SCORE lScore){return true;}
...
public:
	...
	//游戏时间
	virtual DWORD GetDrawStartTime(){return m_dwDrawStartTime;}
	//金币兑换血量
	virtual bool ScoreExchangeBlood(BYTE cbType,DWORD dwBloodPrice,DWORD dwBloodPoint);
	//坐下时间
	virtual DWORD GetSitDownTime(){return m_dwSitDownTime;}
	//
	virtual VOID SetSitDownTime(DWORD dwSitDownTime){m_dwSitDownTime=dwSitDownTime;}
	//自动加分
	virtual bool AutoAddScore();
	// 补分
	virtual bool AutoFillScore();
	//补分
	virtual bool IsAutoVariation(){return m_cbIsAutoVariation;}
	//是否黑户	
	virtual BYTE IsBlackUser(){return m_cbBlackUser;}
	//心跳时间 作废
	virtual DWORD GetHeartTime() { return m_dwHeartTime; }
	//设置心跳 作废
	virtual VOID SetHeartTime( DWORD dwHeartTime){ m_dwHeartTime = dwHeartTime; }
	//是否进桌
	virtual BYTE IsEnterTable() { return m_cbEnterTable; }
	//进桌完成
	virtual VOID SetEnterTable(BYTE cbEnterTable) { m_cbEnterTable = cbEnterTable; }
};
