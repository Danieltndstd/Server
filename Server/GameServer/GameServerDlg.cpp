...
VOID CGameServerDlg::OnCancel()
{
	...
	if (m_ServiceUnits.GetServiceStatus()!=ServiceStatus_Stop)
	{
		...
		int ret = MessageBoxTimeout(m_hWnd,pszQuestion,TEXT("提示"),MB_OKCANCEL|MB_ICONQUESTION,0, 5*1000);
		if( IDCANCEL == ret)
			return ;
	}
	...
	case ServiceStatus_Stop:	//停止状态
		{
			...
			TRACE_LOG(pszDescribe,TraceLevel_Normal);
...
VOID CGameServerDlg::UpdateServerTitle(enServiceStatus ServiceStatus)
{
	...
	_sntprintf(szTitle,CountArray(szTitle),TEXT("[ %s ]--底注[%I64d]--准入[%I64d]-- [ %s ]"),pszServerName
	, m_ModuleInitParameter.GameServiceOption.lCellScore, m_ModuleInitParameter.GameServiceOption.lMinEnterScore, pszStatusName);
...
bool CGameServerDlg::StartServerService(WORD wServerID)
{
	...
	{
		TRACE_LOG(TEXT("房间配置参数不存在或者加载失败"),TraceLevel_Exception);
		return false;
	}
...
VOID CGameServerDlg::OnBnClickedStartService()
{
	...
	try
	{
		LPCTSTR pszDescribe=TEXT("即将开始服务...");
		TRACE_LOG(pszDescribe,TraceLevel_Normal);
		m_ServiceUnits.StartService();
	}
	catch (...)
...
VOID CGameServerDlg::OnBnClickedStopService()
{
	...
	{
		//清除卡线
		//m_ServiceUnits.m_AttemperEngineSink.m_pIRecordDataBaseEngine->PostDataBaseRequest(DBR_GR_KILL_PALAYER_LOCKER,0,NULL,0);
...
VOID CGameServerDlg::OnBnClickedOptionMatch()
{
	...
	CString cmd;
	cmd.Format(TEXT("MatchINITool.exe %d %s"), m_ModuleInitParameter.GameServiceOption.wServerID, 
		m_ModuleInitParameter.GameServiceOption.szServerName);
	CStringA ascCMD(cmd);

	WinExec(ascCMD, SW_SHOW);
}
...
BOOL CGameServerDlg::OnQueryEndSession()
{
	...
	if (m_ServiceUnits.GetServiceStatus()!=ServiceStatus_Stop)
	{
		TRACE_LOG(TEXT("服务正在运行中，系统要求注销回话请求失败"),TraceLevel_Warning);
		return FALSE;
	}
