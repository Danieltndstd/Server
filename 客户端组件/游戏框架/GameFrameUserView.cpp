...
VOID CGameFrameUserView::SetGameBillInfo(WORD wKindID, WORD wServerID)
{
	...
	_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/Ads/GameFrame.htm?KindID=%ld&ServerID=%ld"),szPlatformLink,wKindID,wServerID);
...
VOID CGameFrameUserView::RectifyControl(INT nWidth, INT nHeight)
{
	...
	m_btLoveLiness.SetWindowPos(NULL,nWidth/2 - rcButton.Width()/2,nHeight-rcButton.Height()-6,0,0,SWP_NOZORDER|SWP_NOSIZE);
...
VOID CGameFrameUserView::OnBnClickedUserMedal()
{
	...
	_sntprintf(szLogonLink,CountArray(szLogonLink),TEXT("%s/UserMedal.htm"),szPlatformLink);

	//打开页面
	ShellExecute(NULL,TEXT("OPEN"),szLogonLink,NULL,NULL,SW_NORMAL);

	//CDlgInsurePlaza DlgInsurePlaza;
	//DlgInsurePlaza.DoModal();
...
VOID CGameFrameUserView::OnBnClickedLoveLiness()
{
	...
	//构造地址
	TCHAR szLogonLink[256]=TEXT("");
	_sntprintf(szLogonLink,CountArray(szLogonLink),TEXT("%s/Pay/pay235/game/index.aspx"),szPlatformLink);
...
INT CGameFrameUserView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_btUserMedal.EnableWindow(FALSE);
