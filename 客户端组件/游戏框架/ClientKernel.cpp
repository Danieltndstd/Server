...
#define IPC_SUB_GF_CLIENT_INSURE_SHORT		900							//元宝不足，通知房间
#define IPC_CMD_GR_CONTROL					10							

TCHAR							g_szLogRoot[MAX_PATH];					//日志根目录
//////////////////////////////////////////////////////////////////////////////////
	...
	case IPC_CMD_GF_SOCKET:		//网络消息
		{
			...
			if (bSuccess==false)
			{
				//g_Log.LogToFile(g_szLogRoot,TEXT("主命令:%d, 次命令:%d"),wMainCmdID,wSubCmdID);
				ASSERT(m_pIClientKernelSink!=NULL);
...
VOID CClientKernel::OnUserItemAcitve(IClientUserItem * pIClientUserItem)
{
	...
	for (INT_PTR i=0;i<m_UserEventSinkArray.GetCount();i++)
	{
		...
		//else g_Log.LogToFile(g_szLogRoot,TEXT("获取指针失败"));
	}
...
VOID CClientKernel::OnUserItemDelete(IClientUserItem * pIClientUserItem)
{
	...
	for (INT_PTR i=0;i<m_UserEventSinkArray.GetCount();i++)
	{
		...
		//else g_Log.LogToFile(g_szLogRoot,TEXT("获取指针失败 2"));
	}
...
bool CClientKernel::OnProcessMainSocket(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	if (wSubCmdID==IPC_SUB_GF_SOCKET_RECV)
	{
		...
		//_sntprintf(buffer, sizeof(buffer), TEXT("网络错误 - 主命令：%d, 子命令：%d"), wMainCmdID, wSubCmdID);
...
bool CClientKernel::OnProcessMainControl(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	case IPC_SUB_GF_CLOSE_PROCESS:	//关闭进程
		{
			...
			//关闭进程
			ASSERT(m_pIClientKernelSink!=NULL);
			//g_Log.LogToFile(g_szLogRoot,TEXT("关闭进程"));
			if (m_pIClientKernelSink!=NULL) m_pIClientKernelSink->CloseGameClient();
	...
	//case IPC_SUB_GF_MATCH_READY:	//比赛开始，时间启动
	//	{
	//		CMatchFrameControl::GetInstance()->SetMatchOverStatus(FALSE);
	//		return true;
	//	}
	//case IPC_SUB_GF_MATCH_OUT:		//玩家淘汰，时间停止
	//	{
...
bool CClientKernel::OnProcessMainPropertyInfo(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	case IPC_SUB_GR_PROPERTY_GLAD_MSG: // 滚动消息
		{
	...
	case IPC_SUB_GF_PROPERTY_PING_MSG:
		{
			...
			TCHAR sztmp[32]={0};
			_stprintf(sztmp,_T("网络速度:%d ms"),*pSpeed);
...
bool CClientKernel::OnSocketSubSystemMessage(VOID * pData, WORD wDataSize)
{
	...
	//关闭处理
 	if ((pSystemMessage->wType&SMT_CLOSE_GAME)!=0)
 	{
 		...
 		//中断连接
 		//IntermitConnect();
		//g_Log.LogToFile(g_szLogRoot,TEXT("系统消息通知游戏关闭"));
 	}
...
bool CClientKernel::OnSocketSubActionMessage(VOID * pData, WORD wDataSize)
{
	...
	if ((pActionMessage->wType&SMT_CLOSE_GAME)!=0)
	{
		...
		//中断连接
		//IntermitConnect();
		//g_Log.LogToFile(g_szLogRoot,TEXT("动作消息通知游戏关闭"));
	}
	...
	//if ((m_ServerAttribute.szServerName[0]==0)||(m_UserAttribute.dwUserID==0L))
	{
		if (VERSION_EFFICACY==FALSE)
		{
			...
			else
			{
				...
				{
				case 1:
					_stprintf(szServerName,_T("体验场"));
					break;
				case 2:
					_stprintf(szServerName,_T("新手场"));
					break;
				case 3:
					_stprintf(szServerName,_T("初级场"));
					break;
				case 4:
					_stprintf(szServerName,_T("中级场"));
					break;
				case 5:
					_stprintf(szServerName,_T("高级场"));
					break;
				case 6:
					_stprintf(szServerName,_T("富豪场"));
					break;
				}

				_sntprintf(szTitle,CountArray(szTitle),TEXT("%s->%s->底分%d"),m_GameAttribute.szGameName,szServerName,m_lCellScore);
			}
