...
public:
	...
	//CMD_GR_MatchRewardResult		*m_pMatchResult;					//比赛结果

	//组件变量
public:
	CDlgInsureGame                  m_DlgInsureGame;                    //银行接口
	CDlgGameInsureZJG				m_DlgInsureGameZJG;					//紫金阁银行接口
...
public:
	...
	//用户等级
	WORD GetUserLevelItem(IClientUserItem * pIClientUserItem);
	//等级索引
	WORD GetUserLevelIconIndex(INT nLevel);
...
public:
	...
	//银行取紫金阁
	void ShowInsureGetZJG(IClientKernel* pIClientKernel, CPoint pos);
};
