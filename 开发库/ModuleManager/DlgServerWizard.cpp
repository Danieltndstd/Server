...
BOOL CDlgServerOptionItem1::OnInitDialog()
{
	...
	SetDlgItemInt(IDC_ANROID_COUNT, 0);
	...
	{
		SetDlgItemInt(IDC_ROOMID,m_pGameServiceOption->dwRoomID);
	}
	...
	if (m_pGameServiceOption->wAndroidCount!=0) SetDlgItemInt(IDC_ANROID_COUNT, (LONG)m_pGameServiceOption->wAndroidCount);
...
bool CDlgServerOptionItem1::SaveInputInfo()
{
	...
	m_pGameServiceOption->wAndroidCount=GetDlgItemInt(IDC_ANROID_COUNT);
	...
	m_pGameServiceOption->dwRoomID=GetDlgItemInt(IDC_ROOMID);
...
VOID CDlgServerOptionItem1::InitCtrlWindow()
{
	...
	((CEdit *)GetDlgItem(IDC_ANROID_COUNT))->LimitText(4);
	...
	((CEdit *)GetDlgItem(IDC_ROOMID))->LimitText(10);

	//变量定义
	CComboBox * pComboBox=(CComboBox *)GetDlgItem(IDC_SERVER_TYPE);
	WORD wServerType[3]={GAME_GENRE_GOLD,GAME_GENRE_SCORE,GAME_GENRE_MATCH};
	LPCTSTR pszServerType[3]={TEXT("房间类型"),TEXT("房卡类型"),TEXT("比赛类型")};
...
VOID CDlgServerOptionItem1::UpdateDataBaseName()
{
	...
	{
	case GAME_GENRE_GOLD:	//房间类型
		{
			SetDlgItemText(IDC_DATABASE_NAME,szTreasureDB);
			break;
		}
	case GAME_GENRE_SCORE: //房卡类型
		{
			SetDlgItemText(IDC_DATABASE_NAME,szTreasureDB);
			break;
		}
	case GAME_GENRE_MATCH://比赛类型
		{
			SetDlgItemText(IDC_DATABASE_NAME,TEXT("QPGameMatchDB"));
			break;
		}
	default:
		{
...
BOOL CDlgServerOptionItem2::OnInitDialog()
{
	...
	if (m_pGameServiceOption->lMinEnterTiLi!=0) SetDlgItemInt(IDC_MIN_TABLE_TILI,(INT)m_pGameServiceOption->lMinEnterTiLi);
	...
	//快速加入
	//bool bQuickStart=CServerRule::IsQuickStartMode(m_pGameServiceOption->dwServerRule);
	//((CButton *)GetDlgItem(IDC_QUICK_START_MODE))->SetCheck((bQuickStart==TRUE)?BST_CHECKED:BST_UNCHECKED);
	...
	//((CButton *)GetDlgItem(IDC_IMMEDIATE_WRITE_SCORE))->SetCheck(TRUE);
	//((CButton *)GetDlgItem(IDC_RECORD_GAME_SCORE))->SetCheck(TRUE);
	//((CButton *)GetDlgItem(IDC_ALLOW_ANDROID_ATTEND))->SetCheck(TRUE);
	//((CButton *)GetDlgItem(IDC_ALLOW_ANDROID_SIMULATE))->SetCheck(TRUE);
...
bool CDlgServerOptionItem2::SaveInputInfo()
{
	...
	m_pGameServiceOption->lMinEnterTiLi=GetDlgItemInt(IDC_MIN_TABLE_TILI);
	...
	//快速加入
//	CButton * pQuickStartMode=(CButton *)GetDlgItem(IDC_QUICK_START_MODE);
//	CServerRule::SetQuickStartMode(m_pGameServiceOption->dwServerRule,(pQuickStartMode->GetCheck()==BST_CHECKED));
...
VOID CDlgServerOptionItem2::InitCtrlWindow()
{
	...
	((CEdit *)GetDlgItem(IDC_MIN_TABLE_TILI))->LimitText(8);
	((CEdit *)GetDlgItem(IDC_HIGH_SCORE_GROUP))->LimitText(40);
...
bool CDlgServerWizardItem1::LoadDBModuleItem()
{
	...
	if (m_ModuleInfoManager.LoadGameModuleInfo(m_ModuleInfoBuffer)==true)
	{
		...
		while (Position!=NULL)
		{
			...
			//插入列表
			ASSERT_ERR(pGameModuleInfo!=NULL);
...
bool CDlgServerWizard::InitDefaultGameServiceOption()
{
	...
	//if (ServerInfoManager.LoadGameServerInfo(szMachineID,m_ModuleInitParameter.GameServiceAttrib.wKindID,GameServerResult)==false)
	//{
	//	return false;
	//}
...
VOID CDlgServerWizard::SetWizardParameter(IGameServiceManager * pIGameServiceManager, tagGameServiceOption * pGameServiceOption)
{
	...
	// 设置默认值
	m_ModuleInitParameter.GameServiceAttrib.cbOffLineTrustee = TRUE;
	m_ModuleInitParameter.GameServiceOption.dwServerRule = SR_IMMEDIATE_WRITE_SCORE | SR_RECORD_GAME_SCORE | SR_ALLOW_ANDROID_SIMULATE | SR_ALLOW_ANDROID_ATTEND;

	//获取属性
	ASSERT_ERR(pIGameServiceManager!=NULL);
	pIGameServiceManager->GetServiceAttrib(m_ModuleInitParameter.GameServiceAttrib);

	InitDefaultGameServiceOption();
	//设置接口
	...
	if (pGameServiceOption==NULL)
	{
		...
		if ( m_ModuleInitParameter.GameServiceOption.dwRoomID==0 )
...
{
	//效验参数
	ASSERT_ERR(m_wActiveIndex>0);
	ASSERT_ERR(m_wActiveIndex<CountArray(m_pWizardItem));
...
{
	//效验参数
	ASSERT_ERR((m_wActiveIndex+1)<CountArray(m_pWizardItem));
...
VOID CDlgServerWizard::OnBnClickedFinish()
{
	...
	GameServerCreate.lMinEnterTiLi=m_ModuleInitParameter.GameServiceOption.lMinEnterTiLi;
	...
	GameServerCreate.wAndroidCount=m_ModuleInitParameter.GameServiceOption.wAndroidCount;
	...
	GameServerCreate.dwRoomID=m_ModuleInitParameter.GameServiceOption.dwRoomID;
	...
	//房间配置
	GameServerCreate.lRestrictScore		=m_ModuleInitParameter.GameServiceOption.lRestrictScore		;	//限制积分
	GameServerCreate.lMinTableScore		=m_ModuleInitParameter.GameServiceOption.lMinTableScore		;	//最低积分
	GameServerCreate.lMinEnterScore		=m_ModuleInitParameter.GameServiceOption.lMinEnterScore		;	//最低积分
	GameServerCreate.lMaxEnterScore		=m_ModuleInitParameter.GameServiceOption.lMaxEnterScore		;	//最高积分
	GameServerCreate.lMinEnterTiLi		=m_ModuleInitParameter.GameServiceOption.lMinEnterTiLi		;	//最低体力	

	GameServerCreate.wCardBoxIndex		=m_ModuleInitParameter.GameServiceOption.wCardBoxIndex		;						//宝箱索引
	GameServerCreate.wBloodIndex		=m_ModuleInitParameter.GameServiceOption.wBloodIndex		;
	...
	m_ModuleInitParameter.GameServiceOption.lMinEnterTiLi=GameServerResult.lMinEnterTiLi;


	m_ModuleInitParameter.GameServiceOption.wCardBoxIndex		=GameServerResult.wCardBoxIndex		;						//宝箱索引
	m_ModuleInitParameter.GameServiceOption.wBloodIndex		=GameServerResult.wBloodIndex		;
	...
	m_ModuleInitParameter.GameServiceOption.wAndroidCount=GameServerResult.wAndroidCount;
	...
	m_ModuleInitParameter.GameServiceOption.dwRoomID=GameServerResult.dwRoomID;
	m_ModuleInitParameter.GameServiceOption.wBloodIndex=GameServerResult.wBloodIndex;
	m_ModuleInitParameter.GameServiceOption.wRoomType=GameServerResult.wRoomType;
	m_ModuleInitParameter.GameServiceOption.dwBloodScore=GameServerResult.dwBloodScore;
	_stprintf(m_ModuleInitParameter.GameServiceOption.szServerName,TEXT("%s-%s"),GameServerResult.szGameName,GameServerResult.szServerName);
