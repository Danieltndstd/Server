...
bool CServerListManager::InsertGameServer(tagGameServer * pGameServer)
{
	...
	//设置索引
	//I_TRACE(TEXT("%s,MinScore:%lld,MaxScore:%lld,CellScore:%lld"),pGameServer->szServerName,pGameServer->lMinEnterScore,pGameServer->lMaxEnterScore,pGameServer->lCellScore);
	m_ServerItemMap[pGameServer->wServerID]=pGameServerItem;

	return true;
}

bool CServerListManager::InsertGameMatch(tagGameMatch * pGameMatch)
{
	CGameServerItem * pGameServerItem=SearchGameServer(pGameMatch->wServerID);

	if ( pGameServerItem == NULL) return false;

	ZeroMemory(&pGameServerItem->m_GameMatch,sizeof(tagGameMatch));

	CopyMemory(&pGameServerItem->m_GameMatch,pGameMatch,sizeof(tagGameMatch));
