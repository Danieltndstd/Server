...
//绘画背景
BOOL CFaceSelectControl::OnEraseBkgnd(CDC * pDC)
{
	//获取位置
	CRect rcClient;
	GetClientRect(&rcClient);
	...
	//建立缓冲
	CImage ImageBuffer;
	ImageBuffer.Create(rcClient.Width(),rcClient.Height(),30);
	...
	ImageTitle.LoadImage(AfxGetInstanceHandle(),TEXT("SELECTFACE_TITLE"));
	
	//绘画背景
	ImageTitle.DrawImage(pBufferDC,40,4);
