...
//激活比赛
bool CGlobalInfoManager::ActiveServerItem(WORD wBindIndex, tagGameServer & GameServer, tagGameMatch & GameMatch)
{
	//寻找房间
	CGlobalServerItem * pGlobalServerItem=NULL;
	if (m_MapServerID.Lookup(GameServer.wServerID,pGlobalServerItem)==TRUE)
	...
	pGlobalServerItem->m_wIndex=wBindIndex;
	pGlobalServerItem->m_GameServer=GameServer;
	pGlobalServerItem->m_GameMatch=GameMatch;
...
{
	//效验参数
	ASSERT_ERR(pGlobalUserItem!=NULL);
...
{
	//效验参数
	ASSERT_ERR(pGlobalPlazaItem!=NULL);
...
{
	//效验参数
	ASSERT_ERR(pGlobalServerItem!=NULL);
