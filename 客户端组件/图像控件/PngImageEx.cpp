...
bool CPngImageEx::LoadImageDes(CWnd* pWnd, HINSTANCE hInstance, LPCTSTR pszResourceName)
{
	//加载判断
	ASSERT(m_pImage==NULL);
	if (m_pImage!=NULL) return false;
	...
	//查找资源
	HRSRC hResource=FindResource(hInstance,pszResourceName,TEXT("ENCRYPNG"));
	if (hResource==NULL) return false;

	//读取资源
	DWORD dwImageSize=SizeofResource(hInstance,hResource);
	BYTE* pImageBuffer= (BYTE*)LoadResource(hInstance,hResource);

	//解密
	CAesEncryptor::aes_decry(pImageBuffer, pImageBuffer);
	DecryptData(pImageBuffer, dwImageSize);
