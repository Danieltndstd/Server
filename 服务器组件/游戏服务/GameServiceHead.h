...
// ZZL..............
#define START_MODE_SYSTRM_BANKER	0x12								//系统坐庄（所有人准备时就开始游戏，1个人准备也可以开始）
...
//游戏里数据事件标识
#define DATABASE_CHECKMEDAL			100									//查询奖牌结果
...
struct tagGameServiceOption
{
	...
	WORD							wAndroidCount;						//预加载机器人数目
	WORD							wTableCount;						//桌子数目
	WORD							wServerPort;						//服务端口
	WORD							wServerType;						//房间类型
	//---------------
	DWORD							dwRoomID;							//ROOMID
	WORD							wRoomKind;							//房间种类 1房间 2同玩
	WORD							wRoomType;							//房间类型 0新手 1初级 2中级 3高级
	DWORD							dwBloodScore;						//带入血量
	DWORD							dwBloodLower;						//补血下限
	WORD							wLockPoint;							//幸运点
	BYTE							cbRestTime;							//休息时间
	WORD							wBloodIndex;						//补血索引
	WORD							wCardBoxIndex;						//宝箱索引
	WORD							wRankIndex;							//排名索引
	WORD							wRankTime;							//排名周期
	WORD							wRewardExp;							//每局经验
	WORD							wCashIn;							//每100兑换金币
	WORD							wRoomTimeOut;						//房间关闭时间，同玩房
	BYTE							cbIsKickOut;						//开启踢人
	BYTE							cbIsSendRed;						//开启发红包
	//----------------
...
struct tagAndroidUserParameter
{
	...
	IDataBaseEngine*				pIDabaBaseEngine;					//数据库接口
	WCHAR							guiRoom[40];						//房间id
};
...
// 滚动消息
struct CMD_S_Scroll_Message
{
	TCHAR				szTipMessage[1024];						// 提示信息
};

// 捕鱼战绩信息
struct tagFishRecordInfo
{
	DWORD							dwUserID;							// 用户ID
	DWORD							dwRoomRatio;						// 房间倍率
	DWORD							dwBulletID;							// 子弹ID
	SCORE							dwBulletScore;						// 子弹价值
	DWORD							dwFishID;							// 目标ID
	DWORD							dwFishType;							// 目标类型
	SCORE							dwFishScore;						// 目标价值
	SCORE							dwRevenueScore;						// 抽水分数
	
	DWORD							dwSceneType;						// 场景类型
	DWORD							dwScoreType;						// 结算类型 0 退出结算 1 场景结算 2 开炮 3 打死鱼
	DWORD							dwBulletSum;						// 子弹数量
	SCORE							dwBulletAllScore;					// 子弹总价值
	SCORE							dwCheckFishAllScore;				// 击杀总分数
	SCORE							dwScore;							// 收益分数
	SCORE							dwStartScore;						// 开始筹码
	SCORE							dwEndScore;							// 结束筹码

	SYSTEMTIME						stStartTiem;						// 开始时间
	SYSTEMTIME						stEndTiem;							// 结束时间

	TCHAR							szGameDetails[32];					// 游戏详情介绍
};
...
struct tagScoreInfo
{
	...
	// 2019 - 9 - 5 新增战绩查询字段
	SCORE							lAddScore;							//用户使用分数
	TCHAR							TCardDate[MAX_PATH];				//用户手牌数据
	TCHAR							TGameProce[MAX_PATH];				//当局游戏流程
};
...
{
	BYTE							cbBanker;							//是否地主
	BYTE							cbRobBanker;						//抢地主次数
	BYTE							cbSpring;							//是否春天
	BYTE							cbRocket;							//是否打出火箭
	BYTE							cbBombCount;						//炸弹数量
	WORD							wMultiple;							//倍数
};
...
struct tagUserInfoPlus
{
	...
	//入场条件
	SCORE							lEnterScore;						//入场费用
	SCORE							lMaxEnterScore;						//最高限制
	SCORE							lEnterPower;						//入场体力
	SCORE							lInitBlood;							//初始血量
	BYTE							cbBloodRoom;						//血量房间
	BYTE							cbRoomType;							//房间类型
	BYTE							cbAutoAddScore;						//自动加分
};
...
struct tagMatchPropertyName
{
	DWORD							dwPropertyID;						//道具ID
	TCHAR							MatchPropertyName[128];				//比赛道具
};

#define VIDEO_LEN	30*1024

struct tagVideo
{
	DWORD							dwCode;								//录像值
	BYTE							szFileName[100];					//文件名
	BYTE							cbStream[VIDEO_LEN];				//流						
};
...
interface IGameDataBaseEngineSink : public IUnknownEx
{
	...
	//数据处理
	virtual bool OnGameDataBaseRequest(DWORD dwUserID, WORD wTableID, WORD wChairID)=NULL;
};
...
public:
	...
	// 补充分数
	virtual SCORE GetFillUserScore() = NULL;
...
public:
	//设置状态
	virtual bool SetUserStatus(BYTE cbUserStatus, WORD wTableID, WORD wChairID, BYTE cbNotifyClient=TRUE)=NULL;
	//写入积分
	...
	//写入奖牌
	virtual bool WriteUserMedal(LONG lUserMedal,BYTE cbScoreType)=NULL;
	//获取奖牌
	virtual LONG GetUserMedal()=NULL;
	//修改权限
...
public:
	...
	// 补分
	virtual bool FillUserScore(SCORE lScore, BYTE IsAutoAddScore)=NULL;

	//冻结血量
	virtual bool FrozenedUserBlood(SCORE lScore)=NULL;
	//解冻血量
	virtual bool UnFrozenedUserBlood(SCORE lScore)=NULL;
...
public:
	...
	//游戏时间
	virtual DWORD GetDrawStartTime()=NULL;
	//金币兑换血量
	virtual bool ScoreExchangeBlood(BYTE cbType,DWORD dwBloodPrice,DWORD dwBloodPoint)=NULL;
	//
	virtual tagUserCardBox* GetUserCardBox()=NULL;
	//坐下时间
	virtual DWORD GetSitDownTime()=NULL;
	//
	virtual VOID SetSitDownTime(DWORD dwSitDownTime)=NULL;
	//自动补分
	virtual bool AutoAddScore()=NULL;
	// 补分
	virtual bool AutoFillScore() = NULL;
	//补分判断
	virtual bool IsAutoVariation()=NULL;
	//黑户标识
	virtual BYTE IsBlackUser()=NULL;
	//心跳时间
	virtual DWORD GetHeartTime()=NULL;
	//
	virtual VOID SetHeartTime( DWORD dwHeartTime)=NULL;
	//是否进桌
	virtual BYTE IsEnterTable() = NULL;
	//进桌完成
	virtual VOID SetEnterTable(BYTE cbEnterTable) = NULL;
};
...
interface IServerUserItemSink : public IUnknownEx
{
	...
	//用户状态
	virtual bool OnEventUserItemStatus(IServerUserItem * pIServerUserItem, WORD wOldTableID=INVALID_TABLE, WORD wOldChairID=INVALID_CHAIR, BYTE cbNotifyClient=TRUE)=NULL;
	//用户权限
	...
	//用户任务
	virtual bool OnEventUserItemTask(IServerUserItem *pIServerUserItem,WORD wTask,WORD wCount,BYTE IsSubmit=false)=NULL;
};
...
public:
	// 捕鱼写记录
	virtual bool WriteFishRecord(WORD wChairID, tagFishRecordInfo & FishRecordInfo, BYTE WriteScore = 0) = NULL;
	// 子游戏 滚动消息触发
	virtual bool GameScrollMessage(VOID * pData, WORD wDataSize) = NULL;
	//写入积分
	virtual bool WriteUserScore(WORD wChairID, tagScoreInfo & ScoreInfo, BYTE WriteScore=0)=NULL;
	//写入积分
	virtual bool WriteTableScore(tagScoreInfo ScoreInfoArray[], WORD wScoreCount)=NULL;
	//写入奖牌
	virtual bool WriteUserMedal(WORD wChairID, tagMedalInfo & MedalInfo)=NULL;
	//写入奖牌
	virtual bool WriteTableMedal(tagMedalInfo MedalInfoArray[], WORD wCount)=NULL;
	//录像写入数据库
	virtual bool WriteVideoRecord(tagVideo& video)=NULL;
...
public:
	//起立动作
	virtual bool PerformStandUpAction(IServerUserItem * pIServerUserItem,BYTE cbNotifyClient=TRUE)=NULL;
	//旁观动作
...
public:
	...
	//游戏事件接口
	virtual bool WriteGameEvent(WORD wChairID,void* pData)=NULL;
};
...
public:
	...
	virtual bool SendGameMessage(IServerUserItem * pIServerUserItem, LPCTSTR lpszMessage, WORD wType)=NULL;
	//定时赛通知消息
	virtual bool SendMatchMessage(WORD wMainCmdID, WORD wSubCmdID)=NULL;
	//定时赛通知消息
	virtual bool SendMatchMessage(WORD wMainCmdID, WORD wSubCmdID,VOID * pData, WORD wDataSize)=NULL;
	//宝箱消息
	virtual bool SendUserCardBox(IServerUserItem * pIServerUserItem,bool IsConcludeGame=false)=NULL;
	//网络接口
public:
	//发送数据
...
public:
	...
	//投递事件
	virtual bool PostDataBaseRequest(IServerUserItem* pIServerUserItem, WORD wRequest, VOID *pData, WORD wDataSize) = NULL;
	//投递协调服
	virtual bool SendServerInfo() = NULL;
	//任务投递
	virtual bool SendTaskComplete(DWORD dwUserID, TCHAR* szEventCode) = NULL;
	//游戏事件
	virtual bool PostGameEvent(IServerUserItem* pIServerUserItem,VOID *pData)=NULL;
	//登录失败
	virtual bool SendLogonFailure(LPCTSTR pszString, LONG lErrorCode, DWORD dwSocketID)=NULL;
	//
	virtual void ShutDownSocket(IServerUserItem* pIServerUserItem)=NULL;
};
...
public:
	...
	//获得index
	//virtual WORD GetAndroidIndex()=NULL;
	//获取roud id
	//virtual WORD GetRoundId()=NULL;
...
public:
	...
	//创建机器：个数
	virtual void CreateAndroidUserItem(DWORD dwCount)=NULL;
	//库存数量
	virtual DWORD GetStockCount()=NULL;
	//获取对应机器人
	//virtual IAndroidUserItem* GetAndroidUser(DWORD dwUserID)=NULL;
...
public:
	...
	//状态接口
public:
	//virtual void SetQuickJionTimer(DWORD dwRepeat)=NULL; 
	//机器数目
...
public:
	...
	virtual bool  InitMatchInterface(ITCPNetworkEngineEvent *pTCPNetworkEngine,IDataBaseEngine *pIDataBaseEngine, IServerUserManager *pIServerUserManager,
					IMainServiceFrame *pIMainServiceFrame,ITimerEngine *pITimerEngine,IAndroidUserManager *pAndroidUserManager, WCHAR guidRoom[])=NULL;
	//初始化桌子框架
	virtual bool  InitTableFrame(ITableFrame * pTableFrame,WORD wTableID)=NULL;
...
public:
	//用户参加比赛
	virtual bool  OnUserJoinGame(IServerUserItem * pIServerUserItem)=NULL;
	//用户退出比赛
