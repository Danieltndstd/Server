...
{	
	...
	if (false==m_bBankStorage && (m_lInCount <= 0 || m_lInCount > m_lStorageGold ))
	{
		//构造消息
		CString strMessage;
		if(m_lStorageGold>0)strMessage.Format(TEXT("你输入的亿币值必须在1和%I64d之间"), m_lStorageGold);
	...
	if (true==m_bBankStorage && (m_lInCount <= 0 || m_lInCount > m_lGameGold ))
	{
		//构造消息
		CString strMessage;
		if(m_lGameGold>0)strMessage.Format(TEXT("你输入的亿币值必须在1和%I64d之间"),m_lGameGold );
		else strMessage.Format(TEXT("你的当前亿币数目为0,不能进行存储操作!"));
...
{	
	...
	pDC->DrawText(TEXT("当前元宝："),rcScore,DT_END_ELLIPSIS | DT_CENTER | DT_WORDBREAK);
	CRect rcBank(120,105,200,120);
	pDC->DrawText(TEXT("保险柜元宝："),rcBank,DT_END_ELLIPSIS | DT_CENTER | DT_WORDBREAK);
	CRect rect(0,315,nWidth,nHeight);
	CString strInfo;
	strInfo.Format(TEXT("温馨提示：存入元宝免手续费，取出将扣除%ld‰的手续费"), m_wRevenueTake);
