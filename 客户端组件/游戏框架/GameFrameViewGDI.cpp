...
#define  FACE_COUNT					201									//头像数目    
CPngImageEx CGameFrameViewGDI::m_ImageUserface[201];
...
VOID CGameFrameViewGDI::DrawUserAvatar(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight, IClientUserItem * pIClientUserItem)
{
	...
	if (  RealizeWIN7() && CD2DEngine::GetD2DEngine() )
	{
		...
		{
			CString str;
			str.Format(_T("Face%d"),i);
			m_ImageUserface[i].LoadImage(this,GetModuleHandle(AVATAR_CONTROL_DLL_NAME),str);
		}
		...
		else
		{
			...
			if (bOffLine==false)
			{
				...
				//调整参数
				if (wFaceID>=FACE_COUNT) wFaceID=0;

				//绘画头像
				//INT nXImagePos=wFaceID%wLineCount;
				//INT nYImagePos=(wFaceID/wLineCount)*2L;
				m_ImageUserface[wFaceID].DrawImage(pDC,nXPos,nYPos,nWidth,nHeight,0,0,FACE_CX,FACE_CY);
			}
			else
			{
	...
	case DRAW_MODE_ELONGGATE:	//拉伸模式
		{
			...
			//绘画位图
			if (BitImage.IsRectIntersect(rcDirty,0,0,rcClient.Width(),rcClient.Height()))
			{
...
VOID CGameFrameViewGDI::DrawMatchInfo(CDC * pDC)
{
	...
	if(m_pMatchInfo!=NULL)
	{
		...
		INT nX=rcClient.Width()-ImageMatchInfo.GetWidth();
		ImageMatchInfo.AlphaDrawImage(pDC,nX,0,210);
		pDC->SetTextColor(RGB(255,255,255));
		CString strNum;
		strNum.Format(TEXT("%d"),m_pMatchInfo->wGameCount);
		CRect rectInfo(nX+12,40,nX+22,50);
		CDFontEx::DrawText(this,pDC,12,500,strNum,&rectInfo,RGB(255,255,255),nDrawFormat);
		
		CRect rect(nX+38,10,nX+188,22);
		CDFontEx::DrawText(this,pDC,12,500,m_pMatchInfo->szTitle[0],&rect,RGB(196,221,239),nDrawFormat);
		rect.OffsetRect( 0, 20 );
		CDFontEx::DrawText(this,pDC,12,500,m_pMatchInfo->szTitle[1],&rect,RGB(196,221,239),nDrawFormat);
	
		rect.OffsetRect( 0, 20 );
		CDFontEx::DrawText(this,pDC,12,500,m_pMatchInfo->szTitle[2],&rect,RGB(196,221,239),nDrawFormat);
		
		rect.OffsetRect( 0, 20 );
		CDFontEx::DrawText(this,pDC,12,500,m_pMatchInfo->szTitle[3],&rect,RGB(255,221,35),nDrawFormat);
	}
	...
	if(m_strRollText!="" )
	{
		...
		DrawTextString(pDC,m_strRollText,RGB(196,221,239),RGB(0,0,0),m_rcText);
