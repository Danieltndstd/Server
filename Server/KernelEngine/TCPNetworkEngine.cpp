...
bool CTCPNetworkItem::SendData(WORD wMainCmdID, WORD wSubCmdID, WORD wRountID)
{
	...
	if ( m_ConnectType == WEBSOCKET )
	{
		...
		if (pHead->CommandInfo.wMainCmdID != 0)
		{
			CString str;
			str.Format(TEXT("发送消息 Socket:%d,M_Cmd:%d,S_Cmd:%d,SocketID:%d,m_dwClientIP:%d"),
				m_hSocketHandle, pHead->CommandInfo.wMainCmdID, pHead->CommandInfo.wSubCmdID, m_wIndex, m_dwClientIP);
			m_fileLog.Log(str);
		}
...
bool CTCPNetworkItem::WS_SendRawData(const char* data, int len) {
	...
	if (m_bSendIng == false)
	{
		...
		//g_TraceServiceManager.TraceString(TEXT("发送WebSocket消息成功"), TraceLevel_Exception);
		//设置变量
		m_bSendIng = true;
	}
...
bool CTCPNetworkItem::OnRecvCompleted(COverLappedRecv * pOverLappedRecv, DWORD dwThancferred)
{
	...
	if (m_ConnectType == CHECKING) {
		...
		if (pos == recvChar + m_wRecvSize) { //找不到websocket
	...
	if ( m_ConnectType == HANDSHAKING ) { //websocket client - server进行握手
		char* recvChar = (char*)m_cbRecvBuf;
		...
		if ( success == true ){
			...
			m_wRecvSize = 0;
			//g_TraceServiceManager.TraceString(TEXT("websocket握手完成"), TraceLevel_Exception);
			return RecvData(); 
		}
		else {
			CloseSocket(m_wRountID);
...
bool CTCPNetworkItem::OnCloseCompleted()
{
	...
	g_TraceServiceManager.TraceString(TEXT("关闭完成 "), TraceLevel_Exception);
	//关闭事件
...
// 			}
			...
			case WS_BINARY_FRAME: {
				...
				while (outlen > sizeof(TCP_Head))
				{
					...
					//效验数据
					WORD wPacketSize = pHead->TCPInfo.wPacketSize + 8;
					if (wPacketSize == 0)break;
					...
					// 						{
					// 							TCHAR szBuffer[512];
					// 							_sntprintf(szBuffer, CountArray(szBuffer), TEXT("数据包长度太短 %d,%d,%d,%d,%d,%d,%d,%d"), outbuf[0],
					// 								outbuf[1],
					// 								outbuf[2],
					// 								outbuf[3],
					// 								outbuf[4],
					...
					_sntprintf(szBuffer3, CountArray(szBuffer3),TEXT("[%d-%d %d:%d:%d]M_Cmd:%d,S_Cmd:%d,SocketID:%d"), curTime.wMonth, curTime.wDay, curTime.wHour, curTime.wMinute, curTime.wSecond,
						Command.wMainCmdID,Command.wSubCmdID, m_wIndex);
					...
					{
						CString str;
						str.Format(TEXT("接收消息 Socket:%d,M_Cmd:%d,S_Cmd:%d,SocketID:%d,m_dwClientIP:%d"), 
							m_hSocketHandle, Command.wMainCmdID, Command.wSubCmdID, m_wIndex, m_dwClientIP);
						m_fileLog.Log(str);
...
bool CTCPNetworkEngine::ShutDownSocket(DWORD dwSocketID)
{
	...
	g_TraceServiceManager.TraceString(TEXT("ShutDownSocket "), TraceLevel_Exception);
	//发送请求
	...
	case ASYNCHRONISM_SHUT_DOWN:		//安全关闭
		{
			...
			CString cs;
			cs.Format(TEXT("wIndex:%d,wRountID:%d"),pShutDownSocket->wIndex,pShutDownSocket->wRountID);
	...
	case ASYNCHRONISM_DETECT_SOCKET:	//检测连接
		{
			...
			for (INT_PTR i=0;i<m_TempNetworkItemArray.GetCount();i++)
			{
				//获取连接
				CTCPNetworkItem * pTCPNetworkItem=m_TempNetworkItemArray[i];
				if ( pTCPNetworkItem == NULL ) continue;
				CWHDataLocker ThreadLock(pTCPNetworkItem->GetCriticalSection());
				...
				if (pTCPNetworkItem->IsAllowSendData()==true)
				{
					...
					case DEAD_QUOTIETY:		//死亡连接
						{
							...
							SYSTEMTIME curTime;
							GetLocalTime(&curTime);
							cs.Format(TEXT("[%d-%d %d:%d:%d]关闭连接:%d"), curTime.wMonth, curTime.wDay, curTime.wHour, curTime.wMinute, curTime.wSecond,
								pTCPNetworkItem->GetRountID());
							//g_TraceServiceManager.TraceString(cs, TraceLevel_Exception);
							pTCPNetworkItem->CloseSocket(pTCPNetworkItem->GetRountID());
							break;
						}
 					case DANGER_QUOTIETY:	//危险系数
 						{
							...
							if ( pTCPNetworkItem->m_ConnectType != CTCPNetworkItem::ConnectType::WEBSOCKET)
							{
					...
					default:				//默认处理
						{ 
							if (pTCPNetworkItem->GetIndex() < INDEX_ANDROID)
								pTCPNetworkItem->m_wSurvivalTime--; 
// 							CString cs;
// 							SYSTEMTIME curTime;
// 							GetLocalTime(&curTime);
// 							cs.Format(TEXT("[%d-%d %d:%d:%d]生存次数:%d,wIndex:%d"), curTime.wMonth, curTime.wDay, curTime.wHour, curTime.wMinute, curTime.wSecond,
// 								pTCPNetworkItem->m_wSurvivalTime,pTCPNetworkItem->m_wIndex);
// 							g_TraceServiceManager.TraceString(cs, TraceLevel_Exception);
							break; 
						}
					}
