...
CExpressionManager2 * CExpressionManager2::m_pExpressionManager=NULL;		//对象指针

//////////////////////////////////////////////////////////////////////////////////

//构造函数
CExpression2::CExpression2(CExpressionManager2 *pMgr)
	: m_pMgr(pMgr)
{
	//设置变量
	m_wIndex=INVALID_WORD;
...
//获取路径
bool CExpression2::GetExpressionPath(LPTSTR pszImagePath, WORD wMaxCount)
{


	//构造地址
	_sntprintf(pszImagePath,wMaxCount,TEXT("%s\\%03d.GIF"),m_pMgr->GetDirectory(),m_wIndex);

	return true;
}

//设置表情
bool CExpression2::SetExpressionInfo(WORD wIndex, tagExpressionInfo&ExpressionInfo)
{
	//效验状态
	ASSERT(m_ExpressionItem.IsNull()==true);
	if (m_ExpressionItem.IsNull()==false) return false;
...
void CExpression2::DrawImageFrame(CDC* pDC, INT x, INT y, INT_PTR nFrameIndex)
{
	if (m_ExpressionItem.IsNull()) return;

	m_ExpressionItem.DrawImageFrame(pDC, x, y, nFrameIndex);
}

INT CExpression2::GetImageFrames()
{
	if (m_ExpressionItem.IsNull()) return 0;
	return (INT)m_ExpressionItem.GetFrameCount();
}
//////////////////////////////////////////////////////////////////////////////////

//构造函数
CExpressionManager2::CExpressionManager2()
{
	//设置对象
	ASSERT(m_pExpressionManager==NULL);
	if (m_pExpressionManager==NULL) m_pExpressionManager=this;

	ZeroMemory(m_szDirectory, CountArray(m_szDirectory));
	return;
}
...
//表情根路径
LPCTSTR CExpressionManager2::GetDirectory() const
{
	return m_szDirectory;
} // GetDirectory

//初始表情
bool CExpressionManager2::LoadExpression(LPCTSTR szDirectory)
{
	//工作目录
	lstrcpyn(m_szDirectory, szDirectory, CountArray(m_szDirectory));

	//构造路径
	TCHAR szInfoFile[MAX_PATH]=TEXT("");
	_sntprintf(szInfoFile,CountArray(szInfoFile),TEXT("%s\\Expression.INI"),m_szDirectory);
	...
	do
	{
		...
		{
			//构造表情
			CExpression2 * pExpression=NULL;
			try
			{
				pExpression=new CExpression2(this);
				pExpression->SetExpressionInfo(wIndex++,ExpressionInfo);
			}
			catch (...)
			{
...
//解释表情
bool CExpressionManager2::TranslateString(LPCTSTR pszString, tagTranslateResult2&TranslateResult)
{
	//效验参数
	ASSERT((pszString!=NULL)&&(pszString[0]!=0));
	if ((pszString==NULL)||(pszString[0]==0)) return false;
	...
	for (INT i=0;i<nStringLength-1;i++)
	{
		...
		{
			//获取表情
			CExpression2 * pExpression=m_ExpressionArray[j];
			tagExpressionInfo * pExpressionInfo=&pExpression->m_ExpressionInfo;
...
//表情数目
WORD CExpressionManager2::GetExpressionCount()
{
	return (WORD)m_ExpressionArray.GetCount();
}

//枚举表情
CExpression2 * CExpressionManager2::GetExpressionItem(WORD wIndex)
{
	//效验参数
	ASSERT(wIndex<m_ExpressionArray.GetCount());
	if (wIndex>=m_ExpressionArray.GetCount()) return NULL;
