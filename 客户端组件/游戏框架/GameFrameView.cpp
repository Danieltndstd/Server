...
//用户等级
WORD CGameFrameView::GetUserLevelItem(IClientUserItem * pIClientUserItem)
{
	ASSERT(m_pIGameLevelParser!=NULL);
	if (m_pIGameLevelParser==NULL) return -1;

	return m_pIGameLevelParser->GetUserLevelItem(pIClientUserItem);
}

//等级索引
WORD CGameFrameView::GetUserLevelIconIndex(INT nLevel)
{
	switch (nLevel)
	{
	case 1:
	...
	case SUB_GR_USER_INSURE_INFO:
		{
			...
			//紫金阁取款
			m_DlgInsureGameZJG.m_wRevenueTake		= pUserInsureInfo->wRevenueTake;
			m_DlgInsureGameZJG.m_wRevenueTransfer	= pUserInsureInfo->wRevenueTransfer;
			m_DlgInsureGameZJG.m_bankGold			= pUserInsureInfo->lUserInsure;

			m_DlgInsureGameZJG.UpdateBanker();
		}
		break;
	case SUB_GR_USER_INSURE_SUCCESS:
		{
			...
			//紫金阁消息
			m_DlgInsureGameZJG.m_bankGold = pUserInsureSuccess->lUserInsure;
			m_DlgInsureGameZJG.UpdateBanker();
...
//银行取ZJG
void CGameFrameView::ShowInsureGetZJG(IClientKernel* pIClientKernel, CPoint pos)
{
	ASSERT(pIClientKernel!=NULL);
	if(pIClientKernel==NULL)return;
	...
	if (pServerAttribute->wServerType == GAME_GENRE_MATCH) return;

	if (NULL == m_DlgInsureGameZJG.m_hWnd) m_DlgInsureGameZJG.Create(IDD_ZJG_BAKER_STORAGE, this);

	if (!m_DlgInsureGameZJG.IsWindowVisible())
	{
		m_DlgInsureGameZJG.SetSendInfo(pIClientKernel, pIClientKernel->GetMeUserItem());
		m_DlgInsureGameZJG.SetPostPoint(pos);
		m_DlgInsureGameZJG.SetBankerActionType(false);
		m_DlgInsureGameZJG.ShowWindow(SW_SHOW);
	}
	else
		m_DlgInsureGameZJG.ShowWindow(SW_HIDE);
