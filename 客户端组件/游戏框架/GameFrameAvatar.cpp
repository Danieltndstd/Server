...
CD3DTexture           CGameFrameAvatar::m_D3DImageUserFace[201];          //头像资源
...
CGameFrameAvatar::~CGameFrameAvatar()
{
	...
	for(int i=0;i<201;i++)
	{
		if (m_D3DImageUserFace[i].IsNull()==false) m_D3DImageUserFace[i].Destory();
	}
	return;
}
...
VOID CGameFrameAvatar::Initialization(CD3DDevice * pD3DDevice)
{
	...
	for (int i=0;i<201;i++)
	{
		...
		m_D3DImageUserFace[i].LoadImage(pD3DDevice,AvatarResource.hResInstance,str,TEXT("PNG"));
	}
	//设置变量
...
VOID CGameFrameAvatar::DrawUserAvatar(CD3DDevice * pD3DDevice, INT nXPos, INT nYPos, INT nWidth, INT nHeight, IClientUserItem * pIClientUserItem)
{
	...
	else
	{
		...
		if (pUserInfo->cbUserStatus!=US_OFFLINE)
		{		
			m_D3DImageUserFace[wFaceID].DrawImage(pD3DDevice,nXPos,nYPos,nWidth,nHeight,0,0,FACE_CX,FACE_CY);
		}
		else
		{			
			m_D3DImageUserFace[wFaceID].DrawImage(pD3DDevice,nXPos,nYPos,nWidth,nHeight,0,0,FACE_CX,FACE_CY);
