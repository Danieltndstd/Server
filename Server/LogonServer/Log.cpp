...
#define MAX_LOGFILE_SIZE (1024*1024*10)
...
void gFileLogWrite(const TCHAR* root, const TCHAR* info)
{
	...
	while (g_File == NULL || !isSameTime|| (ftell(g_File) > MAX_LOGFILE_SIZE))
	{
		...
		TCHAR szPath[MAX_PATH] ={0};
		_stprintf_s(szPath, TEXT("%s/%4d_%02d_%02d_file_%d.txt"), root, g_curYear, g_curMon, g_curDay, ++g_iFileCnt);
...
void CLog::LogToFile(const TCHAR* root, const TCHAR* format,...)
{
	...
	va_list ap;
	va_start(ap, format);
	int nLen = _vsntprintf_s(szOutput, 1024, format, ap);
	va_end(ap); 
...
void CFileLog::SetRoot(const TCHAR* format,...)
{
	va_list ap;
	va_start(ap, format);
	int nLen = _vsntprintf_s(m_szRoot, MAX_PATH, format, ap);
	va_end(ap); 
} // SetRoot
...
void CFileLog::Log(const TCHAR* format,...)
{
	...
	while (m_File == NULL || !isSameTime|| (ftell(m_File) > MAX_LOGFILE_SIZE))
	{
		...
		TCHAR szPath[MAX_PATH] ={0};
		_stprintf_s(szPath, TEXT("%s/%4d_%02d_%02d_file_%d.txt"), m_szRoot, m_curYear, m_curMon, m_curDay, ++m_iFileCnt);
