...
{
	//效验参数
	ASSERT_ERR(pGameServerInfo!=NULL);
...
bool CServerInfoManager::LoadGameServerInfo(TCHAR szMachineID[LEN_MACHINE_ID], CServerInfoBuffer & ServerInfoBuffer)
{
	...
	try
	{
		...
		if (PlatformDBAide.ExecuteProcess(TEXT("GSP_GS_LoadGameRoomItem"),true)!=DB_SUCCESS)
		{
			...
			//错误提示
			TRACE_LOG(szDescribeString,TraceLevel_Exception);
...
bool CServerInfoManager::InsertGameServer(tagGameServerCreate * pGameServerCreate, tagGameServerInfo & GameServerResult)
{
	...
	try
	{
		...
		PlatformDBAide.AddParameter(TEXT("@wAndroidCount"),pGameServerCreate->wAndroidCount);
		...
		PlatformDBAide.AddParameter(TEXT("@lMinEnterTiLi"),pGameServerCreate->lMinEnterTiLi);
		PlatformDBAide.AddParameter(TEXT("@dwRoomID"),pGameServerCreate->dwRoomID);
		//输出信息
		TCHAR szDescribeString[128]=TEXT("");
		PlatformDBAide.AddParameterOutput(TEXT("@strErrorDescribe"),szDescribeString,sizeof(szDescribeString),adParamOutput);

		//执行命令
		if (PlatformDBAide.ExecuteProcess(TEXT("GSP_GS_InsertGameRoom1"),true)!=DB_SUCCESS)
		{
			//获取信息
			PlatformDBAide.GetParameter(TEXT("@strErrorDescribe"),szDescribeString,CountArray(szDescribeString));
...
bool CServerInfoManager::ModifyGameServer(tagGameServerCreate * pGameServerCreate, tagGameServerInfo & GameServerResult)
{
	...
	try
	{
		...
		//执行命令
		if (PlatformDBAide.ExecuteProcess(TEXT("GSP_GS_ModifyGameRoom1"),true)!=DB_SUCCESS)
		{
			//获取信息
			PlatformDBAide.GetParameter(TEXT("@strErrorDescribe"),szDescribeString,CountArray(szDescribeString));
...
bool CServerInfoManager::ReadGameServerInfo(CDataBaseAide & PlatformDBAide, tagGameServerInfo & GameServerResult)
{
	...
	GameServerResult.dwNativeVersion=GameServerResult.dwServerVersion;
	...
	GameServerResult.lMinEnterTiLi=PlatformDBAide.GetValue_WORD(TEXT("MinPower"));
	...
	GameServerResult.wAndroidCount=PlatformDBAide.GetValue_WORD(TEXT("AndroidCount"));	
	...
	GameServerResult.dwRoomID=PlatformDBAide.GetValue_DWORD(TEXT("RoomID"));
	do 
	{
		if ( GameServerResult.dwRoomID == 0 ) break;
		if ( GameServerResult.wServerType == GAME_GENRE_MATCH ) break;

		GameServerResult.wRoomKind			= PlatformDBAide.GetValue_WORD(TEXT("wRoomKind"));
		PlatformDBAide.GetValue_String(TEXT("szGameName"),GameServerResult.szGameName,CountArray(GameServerResult.szGameName));
		PlatformDBAide.GetValue_String(TEXT("szRoomName"),GameServerResult.szServerName,CountArray(GameServerResult.szServerName));
		GameServerResult.wRoomType			= PlatformDBAide.GetValue_WORD(TEXT("wRoomType"));
		GameServerResult.lCellScore			= PlatformDBAide.GetValue_LONG(TEXT("lCellScore"));		
		GameServerResult.wRevenueRatio		= PlatformDBAide.GetValue_WORD(TEXT("cbGameRevenue"));
		GameServerResult.lMinEnterScore		= PlatformDBAide.GetValue_LONGLONG(TEXT("lMinEnterScore"));
		GameServerResult.lMaxEnterScore		= PlatformDBAide.GetValue_LONGLONG(TEXT("lMaxEnterScore"));
		GameServerResult.lMinEnterTiLi		= PlatformDBAide.GetValue_WORD(TEXT("lMinPower"));
		GameServerResult.dwBloodScore		= PlatformDBAide.GetValue_DWORD(TEXT("dwBloodCount"));
		GameServerResult.dwBloodLower		= PlatformDBAide.GetValue_DWORD(TEXT("dwBloodLower"));
		GameServerResult.wLockPoint			= PlatformDBAide.GetValue_WORD(TEXT("wLockPoint"));
		GameServerResult.cbRestTime			= PlatformDBAide.GetValue_BYTE(TEXT("cbRestTime"));
		GameServerResult.wBloodIndex		= PlatformDBAide.GetValue_WORD(TEXT("wBloodIndex"));
		GameServerResult.wCardBoxIndex		= PlatformDBAide.GetValue_WORD(TEXT("wDrawBoxIndex"));
		GameServerResult.wRankIndex			= PlatformDBAide.GetValue_WORD(TEXT("wRankIndex"));
		GameServerResult.wRankTime			= PlatformDBAide.GetValue_WORD(TEXT("wRankTime"));
		GameServerResult.wRewardExp			= PlatformDBAide.GetValue_WORD(TEXT("wRewardExp"));
		GameServerResult.wCashIn			= PlatformDBAide.GetValue_WORD(TEXT("wCashIn"));
		GameServerResult.wRoomTimeOut		= PlatformDBAide.GetValue_WORD(TEXT("wRoomTimeOut"));
		GameServerResult.cbIsKickOut		= PlatformDBAide.GetValue_BYTE(TEXT("cbIsKickOut"));
		GameServerResult.cbIsSendRed		= PlatformDBAide.GetValue_BYTE(TEXT("cbIsSendRed"));

		if ( GameServerResult.lMinEnterTiLi == 0 ) //体力房不需要服务费
		GameServerResult.lServiceScore		= PlatformDBAide.GetValue_LONGLONG(TEXT("lMinEnterScore"));
	...
	{
		//获取长度
		ASSERT_ERR(lstrlen(szCustomRule)%2==0);
		...
		for (INT i=0;i<nCustomRuleSize;i++)
		{
			...
			//效验字符
			ASSERT_ERR((cbChar1>=TEXT('0'))&&(cbChar1<=TEXT('9'))||(cbChar1>=TEXT('A'))&&(cbChar1<=TEXT('F')));
			ASSERT_ERR((cbChar2>=TEXT('0'))&&(cbChar2<=TEXT('9'))||(cbChar2>=TEXT('A'))&&(cbChar2<=TEXT('F')));
