...
protected:
	...
	//BOOL							m_bQuickTimer;						//是否是快速加入标识
	//DWORD							m_dwRepeat;							//快速加入次数
	//DWORD							m_dwcrtTimes;						//当前次数
...
public:
	...
	//初始化桌子 删除机器人时，先让他起立
	bool InitTableFrame(ITableFrame * pTableFrame,WORD wTableID);
...
public:
	...
	//创建机器：个数
	virtual	void CreateAndroidUserItem(DWORD dwCount);
	//库存数量
	virtual DWORD GetStockCount() { return m_wStockCount; };
	//获取对应机器人
	//virtual IAndroidUserItem* GetAndroidUser(DWORD dwUserID);
...
public:
	...
	////使机器人快速加入函数
	//virtual void SetQuickJionTimer(DWORD dwRepeat); 
