...
VOID CGameFrameViewD3D::DrawViewImage(CD3DDevice* pD3DDevice, CD3DTexture& D3DTexture, BYTE cbDrawMode)
{
	...
	case DRAW_MODE_CENTENT:		//居中模式
		//绘画位图
		D3DTexture.DrawImage(pD3DDevice, (rcClient.Width() - SizeTexture.cx) / 2, (rcClient.Height() - SizeTexture.cy) / 2);
		break;
...
VOID CGameFrameViewD3D::DrawViewImage(CD3DDevice* pD3DDevice, CD3DSprite& D3DSprite, BYTE cbDrawMode)
{
	...
	case DRAW_MODE_CENTENT:		//居中模式
		//绘画位图
		D3DSprite.DrawImage(pD3DDevice, (rcClient.Width() - SizeTexture.cx) / 2, (rcClient.Height() - SizeTexture.cy) / 2);
		break;
...
{
	//效验状态
	ASSERT(!D3DSprite.IsNull());
	if (D3DSprite.IsNull())
		return;

	//获取大小
	CSize SizeNumber;
	if (D3DSprite.GetWidth() > 180)
		SizeNumber.SetSize(D3DSprite.GetWidth() / 11, D3DSprite.GetHeight());
	else
		SizeNumber.SetSize(D3DSprite.GetWidth() / 10, D3DSprite.GetHeight());

	//变量定义
	LONG lNumberCount = 0;
	LONG lNumberTemp = abs(lNumber);
	...
	for (LONG i = 0; i < lNumberCount; i++)
	{
		if (lNumber < 0 && i == lNumberCount - 1)
		{
			D3DSprite.DrawImage(pD3DDevice, nXDrawPos, nYDrawPos, SizeNumber.cx, SizeNumber.cy, 0, 0);
			nXDrawPos -= SizeNumber.cx;
			continue;
		}

		//绘画号码
		LONG lCellNumber = lNumberTemp % 10L;
		D3DSprite.DrawImage(pD3DDevice, nXDrawPos, nYDrawPos, SizeNumber.cx, SizeNumber.cy, (lCellNumber + 1)*SizeNumber.cx, 0);
...
VOID CGameFrameViewD3D::OnSize(UINT nType, INT cx, INT cy)
{
	...
	if (!m_bInitD3D)
	{
		...
		{
			//配置环境
			if (!m_D3DDirect.CreateD3DDirect() || !m_D3DDevice.CreateD3DDevice(m_hWnd, this))
			{
				ASSERT(FALSE);
				return;
			}
...
VOID CGameFrameViewD3D::DrawMatchInfo(CD3DDevice * pD3DDevice)
{
	...
	{
		auto iBaseX = rcClient.right - 190;
		...
		ImageMatchInfo.DrawImage(pD3DDevice, iBaseX, 0, (BYTE)210);

		CString strNum;
		strNum.Format(TEXT("%d"), m_pMatchInfo->wGameCount);
		DrawTextString(pD3DDevice, strNum, iBaseX + 12, 40, DT_LEFT, D3DCOLOR_XRGB(255, 255, 255));

		DrawTextString(pD3DDevice, m_pMatchInfo->szTitle[0], iBaseX + 38, 10, DT_LEFT, D3DCOLOR_XRGB(196, 221, 239));
		DrawTextString(pD3DDevice, m_pMatchInfo->szTitle[1], iBaseX + 38, 30, DT_LEFT, D3DCOLOR_XRGB(196, 221, 239));
		DrawTextString(pD3DDevice, m_pMatchInfo->szTitle[2], iBaseX + 38, 50, DT_LEFT, D3DCOLOR_XRGB(196, 221, 239));
		DrawTextString(pD3DDevice, m_pMatchInfo->szTitle[3], iBaseX + 38, 70, DT_LEFT, D3DCOLOR_XRGB(255, 221, 35));
	}

	auto pGameEngine = CGameFrameEngine::GetInstance();
	auto isFree = pGameEngine->GetGameStatus() == GAME_STATUS_FREE;
	...
	if (m_strRollText.GetLength() > 0)
	{
		...
		D3DCOLOR textColor = D3DCOLOR_XRGB(255, 225, 0);
		...
		if (nTextLine == 3)
		{
			m_D3DRollFont.DrawText(pD3DDevice, szSub1, &m_rcText, DT_CENTER, textColor);
			CRect rcLine = m_rcText;
			rcLine.top += 25;
			if (rcLine.top < rcLine.bottom)
				m_D3DRollFont.DrawText(pD3DDevice, szSub2, &rcLine, DT_CENTER, textColor);
			rcLine.top += 25;
			if (rcLine.top < rcLine.bottom)
				m_D3DRollFont.DrawText(pD3DDevice, szSub3, &rcLine, DT_CENTER, textColor);
		}
		else if (nTextLine == 2)
		{
			m_D3DRollFont.DrawText(pD3DDevice, szSub1, &m_rcText, DT_CENTER, textColor);
		...
		{
			m_D3DRollFont.DrawText(pD3DDevice, m_strRollText, &m_rcText, DT_CENTER, textColor);
		}
	}
}
